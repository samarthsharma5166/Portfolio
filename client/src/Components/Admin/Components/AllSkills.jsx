  ...
  return (
   <div>
    ...
    <div className='flex flex-col justify-center items-center h-[80vh] gap-3'>
    {
      skills.map((skill)=>{
        return(
          <div key={skill._id} className='flex justify-between items-center text-white px-3 py-2 border rounded-full w-[80vw] lg:w-[30vw]'>
            <span>{skill.skill}</span>
            <progress max={100} value={skill.level *20} className='progress-bar' ></progress>
            <MdDelete className='hover:scale-125 transition-all ease-in-out delay-100 cursor-pointer hover:text-red-400' onClick={()=>deleteSkill(skill._id)}></MdDelete>
          </div>
        )
      })
    }
